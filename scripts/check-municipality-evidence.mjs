...
function extractLinkUrls(frontmatter, fieldName) {
  ...
  if (!block) return [];
  const entries = [];
  const matches = [...block.matchAll(/label:\s*"([^"]+)"[\s\S]*?url:\s*"([^"]+)"/g)];
...
function extractBlock(frontmatter, fieldName) {
  return frontmatter.match(new RegExp(`^${fieldName}:\\r?\\n([\\s\\S]*?)(?=^\\w|\\Z)`, 'm'))?.[1] ?? '';
}
...
function inferKind(label) {
  ...
  if (/prøver|kunnskapskrav/.test(source)) return 'exam';
  ...
  if (/regler|vilkår/.test(source)) return 'rules';
  if (/salg, servering og skjenking/.test(source)) return 'serviceHub';
  if (/søke bevilling|søknad/.test(source)) return 'application';
  return 'general';
}
